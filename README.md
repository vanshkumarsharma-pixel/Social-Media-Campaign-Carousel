Campaign Theme
Objective:-Social Media Campaign (Carousel + Motion Posts)
Tools Used:
1 Canva
2 Adode illuatrator
Brand: Eco-Friendly Water Bottle Startup (Example)
Goal: Increase brand awareness, engagement, and sales.
Platform: Instagram, Facebook, LinkedIn
Day 1 – Brand Introduction
Static Carousel (5 Slides)
1 W2elcome to the Brand
2 Our Mission
3 Why Sustainability Matters
4 Product Benefits
5 Follow Us
# Motion Post
Logo reveal animation
Smooth zoom transitions
Animated text: "Hydrate Sustainably"
# Design Tips
Use brand colors consistently
Add icons and illustrations
Keep text minimal
Day 2 – Product Features
1 Static Carousel
2 Product Overview
3 BPA-Free Material
4 Leak-Proof Design
5 emperature Retention
Call-to-Action
# Motion Post
Product rotating animation
Feature callouts appearing one by one
End with "Shop Now"
# After Effects Animation
Position keyframes
Scale animations
Easy Ease transitions
Day 3 – Customer Testimonial
Static Carousel
1 Customer Photo
2 Review Quote
3 Product in Use
4 Rating Slide
5 CTA
# Motion Post
Quote typing effect
Animated star ratings
Customer image reveal
Day 4 – Educational Content
Static Carousel
Topic: Plastic Waste Statistics
1 Problem Overview
2 Statistics
3 Environmental Impact
4 Solutions
5 Brand Contribution
# Motion Post
Animated infographic
Number counters
Graph animations
Day 5 – Behind the Scenes
Static Carousel
1 Team Introduction
2 Manufacturing Process
3 Quality Check
4 Packaging
5 Team Message
# Motion Post
Quick video montage
Smooth slide transitions
Text overlays
Day 6 – Promotional Offer
Static Carousel
1 Special Offer
2 Discount Details
3 Product Showcase
4 Benefits
5 CTA
# Motion Post
Countdown timer
Flashing discount badge
Animated CTA button
Day 7 – Community Engagement
Static Carousel
1 Ask a Question
2 Poll
3 User-Generated Content
4 Challenge Announcement
5 Winner Information
# Motion Post
Animated stickers
Poll indicators
Dynamic text transitionTools Used:

Canva
Adobe Illustrator
Adobe After Effects
